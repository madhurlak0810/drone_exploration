...
ros2 launch gazebo_ros gazebo.launch.py gui:=false world:=install/drone_rl/share/drone_rl/worlds/scattered_world.world &
GAZEBO_PID=$!
...
echo "   Attempting to start GUI client (optional)..."
(gzclient --gui-client-plugin=libgazebo_ros_eol_gui.so > /dev/null 2>&1 &) || echo "   GUI client failed (continuing headless)"
...
echo "Step 4: Initializing SLAM System..."
...
echo "   - Mapping: Real-time SLAM with occupancy grid"
...
echo "   - Start visualization: ./launch_static_map.sh (in separate terminal)"
...
echo ""
echo "Q-learning agent stopped. Shutting down system..."
cleanup